...
$(function() {
  ...
  function fetchData(projectId) {
    ...
    $.ajax({
      ...
      success: function(response) {
        ...
        for (var i=0; i<response.length; i++) {
          ...
          for (var j=0; j<metrics.length; j++) {
            ...
            if (!metricKeys.hasOwnProperty(metrics[j].key)) {
              ...
              if(!filteredMetricKeys.hasOwnProperty(metrics[j].key) && !metrics[j].key.startsWith("_")){
                  filteredMetricKeys[metrics[j].key] = metrics[j].key;
              }
            }
          }
        ...
        filteredMetricKeys = Object.values(filteredMetricKeys);
  ...
  */
  ...
  function selectInit() {
    ...
    // add metric keys
    for (var i=0; i<filteredMetricKeys.length; i++) {
      $('.y-axis').append(new Option(filteredMetricKeys[i], filteredMetricKeys[i]));
      $('.metrics-sort').append(new Option(filteredMetricKeys[i], filteredMetricKeys[i]));
    }
  ...
  function vegaInit() {
    ...
    for (var i=0; i<filteredMetricKeys.length; i++) {
      legendWidth = Math.max(legendWidth, filteredMetricKeys[i].length);
    }
      ...
      "data": [
        {
          ...
          "transform": [
            ...
            {"type": "fold", "fields": filteredMetricKeys}
          ]
        },
        {
          "name": "filtered",
