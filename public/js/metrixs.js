$(function() {
  ...
  $(document).on('click', '.md-metrixs-trigger', function(event) {
    ...
    $('.md-metrixs').html("");
    $('.md-metrixs').append($(
            '<div id="metrixs-view"></div>'+
            '<div style="display:inline-block; float:left" id="matrix-container"></div>' +
            '<div style="display:inline-block; float:left" id="legend"></div>'
	));
    $('#modal-3').addClass('md-show');
    metrixs($(this).data('tp'), $(this).data('fn'), $(this).data('fp'), $(this).data('tn'));
  });

  function metrixs(tp, fn, fp, tn){
  ...
  function tabulate(data, columns) {
    var table = d3.select("#metrixs-view").append("table")
            .attr("style", "margin-left: " + margin.left +"px;" + "margin-top: " + margin.top +"px"),
